...
for conv = {@single,@double}
  vl_twister('state',0) ;
  conv = conv{1} ;
  [w b] = vl_svmpegasos(conv(s.X), int8(s.y), s.lambda, ...
...
for conv = {@single,@double}
  ...
  vl_twister('state',0) ;
  [w1 b1] = vl_svmpegasos(conv(s.X), int8(s.y), s.lambda, ...
                 'StartingIteration', 1, ...
                 'MaxIterations', 1500, ...
                  'BiasMultiplier', s.biasMultiplier) ;
  [w2 b2] = vl_svmpegasos(conv(s.X), int8(s.y), s.lambda, ...
...
for conv = {@single,@double}
  ...
  [we be] = vl_svmpegasos(sxe, int8(s.y), s.lambda, ...
                 'MaxIterations', 100000, ...
                 'BiasMultiplier', s.biasMultiplier, ...
                 'BiasLearningRate', .1) ;
  [w b] = vl_svmpegasos(s.X, int8(s.y), s.lambda, ...
                 'MaxIterations', 100000, ...
                 'BiasMultiplier', s.biasMultiplier, ...
                 'BiasLearningRate', .1,...
                 'homkermap',1,...
...
for conv = {@single,@double}
  ...
  [w b] = vl_svmpegasos(s.X, int8(s.y), s.lambda, ...
                        ...
                        'BiasMultiplier', s.biasMultiplier, ...
                        'BiasLearningRate', .1) ;
  [wd bd] = vl_svmpegasos(s.X, int8(s.y), s.lambda, ...
                          'MaxIterations', 100000, ...
                          'BiasMultiplier', s.biasMultiplier, ...
                          'BiasLearningRate', .1,...
  ...
  vl_assert_almost_equal([w; b], [wd; bd], 1e-2) ;
end
...
for conv = {@single,@double}
  ...
  [w b info] = vl_svmpegasos(s.X, int8(s.y), s.lambda, ...
                        'MaxIterations', 100000, ...
                        'BiasMultiplier', s.biasMultiplier, ...
                        'BiasLearningRate', .1) ;
  [we be infoe] = vl_svmpegasos(s.X, int8(s.y), s.lambda, ...
                          'MaxIterations', 100000, ...
                          'Epsilon',1e-6,...
                          'BiasMultiplier', s.biasMultiplier, ...
                          'BiasLearningRate', .1) ;
