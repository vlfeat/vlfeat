...
The above type of functions define the two typical operations performed by SVM solvers on the training data. Any particular application can define its own data representation and implement the above functions. This allows the use of the SVM solvers implemented in VLFeat.

For double and float arrays we provide the implementation of the aforementioned functions.

We define also an utility struct @ref VlSvmDataset that implements the online expansions of data points. This allows the use of techniques such homogeneous kernel maps @cite{vedaldi11efficient} inside the SVM solver.
*/
 ...
 **
 ** The function performs the inner product between the @a element data point and the model @a model.
 **
 ** See the @ref svmdataset-overview overview for details.
 **/
 ...
 **
 **
 ** The function adds to the model @a model the data point selected by @element multiplied by @multiplier.
 **
 ** See the @ref svmdataset-overview overview for details.
 **/
